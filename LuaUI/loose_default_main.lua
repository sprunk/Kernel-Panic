...
-- KP fix: as of writing this, engine doesn't actually put `rml_setup.lua`
-- in basecontent, so any game that doesn't put it in ZIP manually but uses
-- the default loose `main.lua` will break. KP doesn't care about RML so I
-- just removed the line below with no replacement or compat.
-- VFS.Include(LUAUI_DIRNAME .. "rml_setup.lua",  nil, VFS.ZIP)

-- This one is also broken/fixed same way (VFS mode was ZIP)
VFS.Include(LUAUI_DIRNAME .. 'utils.lua', nil, VFS.RAW_FIRST)
