...
local function DoTheRestart(startscriptfilename)
	...
	if Spring.Reload then
		Spring.Echo(widget:GetInfo().name..": Ok, calling Spring.Reload(\"<start script content>\") now!")
...
local function ListMap(ReturnTo)
	...
	AddFrame(GetMinimapFromActiveMap,{x=vsx*0.5,y=vsy*0.5},vsy/48,{1,0.6,0,1},"cc","c",ReturnTo)
	...
	for m,_ in ipairs(ModSpecific.MapsList) do
		...
		local x,y,side
		if m<=#ModSpecific.MapsList/2 then
			x=vsx*0.1
			side="l"
			y=vsy*(0.85-0.1*(m-1))
		else
			x=vsx*0.9
			side="r"
			y=vsy*(0.85-0.1*(m-1-#ModSpecific.MapsList/2))
		end
		AddFrame(ModSpecific.MapsList[m].ClearName..(missing and " - Missing!" or ""),
			{x=x,y=y},vsy/28,{0,1,0,0.5},side.."c","c",
...
local function SinglePlayer()
	...
	AddFrame("Spectate",{x=vsx*0.10,y=vsy*0.65},vsy/24,{0.33,0.88,1,0.5},"lc","c",ChangeGrouping,0)
	AddFrame("Duel",{x=vsx*0.10,y=vsy*0.55},vsy/24,{0.33,0.88,0,0.5},"lc","c",ChangeGrouping,1)
	AddFrame("Team Game",{x=vsx*0.10,y=vsy*0.45},vsy/24,{0.88,0.79,0,0.5},"lc","c",ChangeGrouping,2)
	AddFrame("Outgunned",{x=vsx*0.10,y=vsy*0.35},vsy/24,{0.88,0.40,0,0.5},"lc","c",ChangeGrouping,3)
	AddFrame("Heroic",{x=vsx*0.10,y=vsy*0.25},vsy/24,{0.88,0,0.88,0.5},"lc","c",ChangeGrouping,4)

	AddFrame("Easy",{x=vsx*0.90,y=vsy*0.60},vsy/24,{0.33,0.88,1,0.5},"rc","c",ChangeDifficulty,1)
	AddFrame("Medium",{x=vsx*0.90,y=vsy*0.50},vsy/24,{0.33,0.88,0,0.5},"rc","c",ChangeDifficulty,2)
	AddFrame("Hard",{x=vsx*0.90,y=vsy*0.40},vsy/24,{0.88,0.79,0,0.5},"rc","c",ChangeDifficulty,3)
	AddFrame("Extreme",{x=vsx*0.90,y=vsy*0.30},vsy/24,{0.88,0.40,0,0.5},"rc","c",ChangeDifficulty,4)
...
local function MainMenu()
	...
	--AddFrame("MultiPlayer",{x=vsx*0.46,y=vsy*0.83},vsy/20,{0,1,0,0.5},"rb","c",FunctionsList.MultiPlayer)
	--AddFrame("Options",{x=vsx*0.54,y=vsy*0.82},vsy/20,{0,1,0,0.5},"lb","c",FunctionsList.Settings)
	AddFrame("Missions",{x=vsx*0.46,y=vsy*0.7},vsy/20,{0,1,0,0.5},"rb","c",FunctionsList.MissionsMenu)
	AddFrame("Skirmish",{x=vsx*0.54,y=vsy*0.6},vsy/20,{0,1,0,0.5},"lb","c",FunctionsList.RandomSinglePlayer)
	AddFrame("Load",{x=vsx*0.46,y=vsy*0.5},vsy/20,{0,1,0,0.5},"rb","c",FunctionsList.PseudoLoad)
	if not (Spring.GetModOptions()["ons"] == nil) then
		...
		if not isDead and not Spring.GetSpectatingState() then
			AddFrame("Save",{x=vsx*0.54,y=vsy*0.4},vsy/20,{0,1,0,0.5},"lb","c",FunctionsList.PseudoSave)
		end
		AddFrame("Restart",{x=vsx*0.46,y=vsy*0.3},vsy/20,{0,1,0,0.5},"rb","c",FunctionsList.RestartSameGame)
	end
	AddFrame("Credits",{x=vsx*0.54,y=vsy*0.2},vsy/20,{0,1,0,0.5},"lb","c",FunctionsList.Credits)
	AddFrame("Readme",{x=vsx*0.46,y=vsy*0.1},vsy/20,{0,1,0,0.5},"rb","c",FunctionsList.PrintReadMe)
	AddFrame("Quit",{x=vsx*0.54,y=vsy*0.0},vsy/20,{0,1,0,0.5},"lb","c",FunctionsList.Quit)
end
...
local function DrawFrame(Frame)
	...
	if type(Text)=="table" then
		Texture,Text,TextureWidth,TextureHeight=Text.texture,Text.text,Text.width,Text.height
		bg_red = Frame.Color[1] or 1
		bg_green = Frame.Color[2] or 1
		bg_blue = Frame.Color[3] or 1
		bg_alpha = Frame.Color[4] or 0.5
	end
	...
	if Game.version and tonumber(string.sub(Game.version,1,4))<=0.785 and string.sub(Game.version,1,5)~="0.78+" then
	...
	if Texture then
		...
		-- Only used for minimaps for now, so hardcoding size and no tint
		xFramedTextSize,yFramedTextSize=0.6*vsy*TextureInfo.xsize/TextureInfo.ysize,0.6*vsy
		bg_red = 1
		bg_green = 1
		bg_blue = 1
	end
	...
	if Texture then
		...
		local FutureEdge=FontSize/3 -- whatever you want
		local Height=math.abs(yPosMax-yPosMin) -- don't touch
		gl.Shape(GL.TRIANGLE_STRIP,{
			{v={xPosMax+Height-FutureEdge,yPosMax-FutureEdge}},
			{v={xPosMax+Height-2*FutureEdge,yPosMax}},
			{v={xPosMax,yPosMin}},
			{v={xPosMin,yPosMax}},
			{v={xPosMin-Height+2*FutureEdge,yPosMin}},
			{v={xPosMin-Height+FutureEdge,yPosMin+FutureEdge}},
			})
		local w=math.max(2,FutureEdge/4)
		gl.Color(bg_red,bg_green,bg_blue,(1+bg_alpha)/2)
		gl.LineWidth(w)
		gl.Shape(GL.LINE_LOOP,{
			{v={xPosMax+Height-2*FutureEdge-w,yPosMax-w}},
			{v={xPosMax+Height-FutureEdge-2*w,yPosMax-FutureEdge}},
			{v={xPosMax-w,yPosMin+w}},
			{v={xPosMin-Height+2*FutureEdge+w,yPosMin+w}},
			{v={xPosMin-Height+FutureEdge+2*w,yPosMin+FutureEdge}},
			{v={xPosMin+w,yPosMax-w}},})
	end
	gl.Color(1,1,1,bg_alpha)
	for n,_ in ipairs(txtLinesList) do
		if TextPosition=="c" then
			gl.Text(txtLinesList[n], (xPosMin+xPosMax)/2, yPosMax+(-0.6-n)*FontSize, FontSize, "cod")
		elseif TextPosition=="r" then
			gl.Text(txtLinesList[n], xPosMax-FontSize/2, yPosMax+(-0.6-n)*FontSize, FontSize, "rod")
		else
			gl.Text(txtLinesList[n], xPosMin+FontSize/2, yPosMax+(-0.6-n)*FontSize, FontSize, "od")
		end
	end
	if Frame.Index==ActiveFrameIndex then
		if Texture then
			...
			local FutureEdge=FontSize/4 -- whatever you want
			local Height=math.abs(yPosMax-yPosMin) -- don't touch
			gl.Color((3+bg_red)/4,(3+bg_green)/4,(3+bg_blue)/4,1)
			gl.LineWidth(math.max(4,FutureEdge/2))
			gl.Shape(GL.LINE_LOOP,{
				--{v={xPosMax,yPosMax}},
				{v={xPosMax+Height-2*FutureEdge,yPosMax}},
				{v={xPosMax+Height-FutureEdge,yPosMax-FutureEdge}},
			...
			gl.Shape(GL.LINE_LOOP,{
				...
				{v={xPosMin+w,yPosMax-w}},})
			gl.Shape(GL.LINE_LOOP,{
				{v={xPosMax+Height-2*FutureEdge-w/2,yPosMax-w/2}},
				{v={xPosMax+Height-FutureEdge-w,yPosMax-FutureEdge}},
				{v={xPosMax-w/2,yPosMin+w/2}},
				{v={xPosMin-Height+2*FutureEdge+w/2,yPosMin+w/2}},
				{v={xPosMin-Height+FutureEdge+w,yPosMin+FutureEdge}},
				{v={xPosMin+w/2,yPosMax-w/2}},})
		end
	end
	gl.LineWidth(1)
	gl.Color(1,1,1,1)
...
local function EitherDrawScreen(self)
	...
	gl.Color(1,1,1,1)
	gl.Text("Spring\n"..(Engine and (Engine.versionFull or Engine.version) or Game.version),4,4,12,"ob")
