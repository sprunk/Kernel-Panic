...
local function ONSCmd(Cmd,Line,WordList,Player)
		...
		if PlayerTeamID and PlayerIsActive and not PlayerIsSpec then
			...
			elseif tonumber(words[1])==1 then
				if DisabledForTeam[PlayerTeamID] then
					local _,_,_,_,_,_,CustomTeamOptions=Spring.GetOldTeamInfo(PlayerTeamID)
	...
	elseif #words==2 then
		if not Spring.IsCheatingEnabled() then
			...
			if t and Spring.GetOldTeamInfo(t) then
				local names=nil
...
function gadget:GameStart()
	...
	for _,TeamID in ipairs(Spring.GetTeamList()) do
		local _,_,_,_,_,_,CustomTeamOptions=Spring.GetOldTeamInfo(TeamID)
