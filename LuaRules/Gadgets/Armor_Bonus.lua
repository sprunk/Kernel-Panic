...
if (gadgetHandler:IsSyncedCode()) then
	...
	function gadget:Initialize()
		...
		for _,team in ipairs(Spring.GetTeamList()) do
			-- Warning: between Spring 104 and 105, incomeMultiplier and customTeamKeys switched position in GetTeamInfo returned values
			local incomeMultiplier = select(8,Spring.GetTeamInfo(team))
			if type(incomeMultiplier)~="number" then
				incomeMultiplier = select(7,Spring.GetTeamInfo(team))
			end
