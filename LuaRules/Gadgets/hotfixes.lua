...
-- Recopy Engine fields into Game
-- Because I can't stress how silly it is to break at every version the interface to know the version
if Engine then
...
-- Restore behavior of GetTeamInfo which changed between 104 and 105
Spring.GetOldTeamInfo = function(team)
		...
		-- Notice how the last two arguments switched position!
		local TeamInfo={Spring.GetTeamInfo(team)}
		if type(TeamInfo[8])=="table" then
			local tmp=TeamInfo[8]
			TeamInfo[8]=TeamInfo[7]
			TeamInfo[7]=tmp
		end
		return unpack(TeamInfo)
	end
