...
function gadget:CommandFallback(u,ud,team,cmd,param,opts)
	if cmd == CMD_DISPATCH then
		...
		while index < 12 and bufferSize[team] > 0 do
			...
			-- For some unclear reason, on Hex Farm packets are not buildable but badblock are
			local blocking_p, feature = Spring.TestBuildOrder(UnitDefNames["packet"].id,tx,y,tz,offsetList[3*index +3])
			local blocking_b, feature = Spring.TestBuildOrder(UnitDefNames["badblock"].id,tx,y,tz,offsetList[3*index +3])
			if not Spring.GetGroundBlocked(tx-8, tz-8, tx+8, tz+8) and (blocking_p==2 or blocking_b==2) and feature==nil then
