...
--SYNCED
	...
	local function GetCustomTeamOptions(team)
		-- Up to Spring 104, CustomTeamOptions is the 7th returned value from Spring.GetTeamInfo(team)
		-- From  Spring 105, CustomTeamOptions is the 8th returned value from Spring.GetTeamInfo(team)
		local TeamInfo={Spring.GetTeamInfo(team)}
		if type(TeamInfo[7])=="table" then
			return TeamInfo[7]
		elseif type(TeamInfo[8])=="table" then
			return TeamInfo[8]
		else
			error("Cannot find Custom Team Options!")
		end
	end
	...
	local function SaveRegenData(team)
		...
		local CustomTeamOptions=GetCustomTeamOptions(team)
		if (string.lower(Spring.GetTeamLuaAI(team) or "")==string.lower(gadget:GetInfo().name)) or
