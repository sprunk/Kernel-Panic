...
function CutOrAddMapSuffix(name)-- Not local, because also called by kp_spring_direct_launch.lua
	local version=(Engine and Engine.version) or (Game and Game.version) or "?"
	local version=tonumber(string.match(version,"^(%d+%.%d+)") or string.match(version,"^(%d+)") or 100)
		+(string.match(version,"^[%d%.]+(%+)") and 0.009 or 0)
...
function FillModSpecific()
	...
	-- ModSpecific.ModFileName="Kernel_Panic_4.9.sd7"
	...
	ModSpecific.FrontEndExecutableFileName="Kernel_Panic_4.8_Launcher_Windows.exe"
