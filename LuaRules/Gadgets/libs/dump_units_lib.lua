...
function SetModOptionsToFull(fullstartscript)
	...
	-- Overwrite Spring.GetModOptions()
	...
	Spring.GetModOptions=function() return FullModOptions end

	-- Warning,  Spring.GetTeamInfo changed between Spring 104 and Spring 105:
	-- Up to Spring 104, Spring.GetTeamInfo(team) would return: nil | number teamID, number leader, bool isDead, bool isAiTeam, string "side", number allyTeam, table customTeamKeys, number incomeMultiplier
	-- From  Spring 105, Spring.GetTeamInfo(team) would return: nil | number teamID, number leader, bool isDead, bool isAiTeam, string "side", number allyTeam, number incomeMultiplier, table customTeamKeys
	-- Spring.GetOldTeamInfo is my own version that always return like in Spring 104

	-- Overwrite Spring.GetTeamInfo()
	local FullTeamInfo={}
	for _,t in ipairs(Spring.GetTeamList()) do
		FullTeamInfo[t]={Spring.GetTeamInfo(t)}
		local ctkidx=false-- Custom Team Keys Index (the only table)
		for k,v in ipairs(FullTeamInfo[t]) do
			if type(v)=="table" then
				if ctkidx then
					Spring.Echo("The "..ctkidx.."th and "..k.."th returned values of GetTeamInfo are both tables!")
				else
					ctkidx=k
				end
			end
		end
		-- Recopy mod options into team custom options, except for keys already set
		for key,value in pairs(FullTeamModOptions[t]) do
			if FullTeamInfo[t][ctkidx][key]==nil then
				FullTeamInfo[t][ctkidx][key]=value
			end
		end
		FullTeamInfo[t][ctkidx].PreviousGetTeamInfoFunc=Spring.GetTeamInfo
	end
	FullTeamInfo.PreviousGetTeamInfoFunc=Spring.GetTeamInfo
	Spring.GetTeamInfo=function(t) return unpack(FullTeamInfo[t]) end

	-- Spring.GetOldTeamInfo() gets its info from Spring.GetTeamInfo
	-- so it doesn't need to be overwritten
end
...
function SpawnAllUnits()
	...
	local UnitList={}
	for _,t in ipairs(Spring.GetTeamList()) do
		local _,_,_,_,_,_,CustomTeamOptions=Spring.GetOldTeamInfo(t)
...
function SpawnResources()
	for _,team in ipairs(Spring.GetTeamList()) do
		local _,_,_,_,_,_,CustomTeamOptions=Spring.GetOldTeamInfo(team)
		if CustomTeamOptions then
...
function GenerateFromCurrentState(Abbreviated,Grid,OnlyStartScript,WithHeightMap)
	...
	for p=1, script.NbrPlayers do
		local teamNum,leader,isDead,isAiTeam,side,allyTeam,teamCustomOptions,incomeMultiplier = Spring.GetOldTeamInfo(TeamFromPlayer[p])
		script.players[p]={}
